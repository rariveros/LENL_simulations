...
if __name__ == '__main__':
    ...
    #scat_1 = ax.scatter(1, 0, s=300, c=np.array([230, 50, 50, 255]) / 255, zorder=10)
    ...
    x1 = 1.2 * x0 * np.cos(2 * np.pi * freq1 * t_grid)
    y1 = -np.sqrt(R ** 2 - x1 ** 2)
    x2 = 0.3 * x0 * np.cos(2 * np.pi * freq1 * t_grid)
    y2 = -np.sqrt(R ** 2 - x2 ** 2)
    ...
    #ani.save("broken_PT.gif", dpi=300, writer=PillowWriter(fps=60))
    #plt.close()
    ...
    plt.savefig("test_02.png", dpi=300)
