...
if __name__ == "__main__":
    ...
    for i in range(N_dirs):
        if i == 0:
            X = np.loadtxt(dirs[i] + '/X_mm.txt', delimiter=',')
            T = np.loadtxt(dirs[i] + '/T_s.txt', delimiter=',')
            Z = np.loadtxt(dirs[i] + '/Z_mm.txt', delimiter=',')
        else:
            X = np.loadtxt(dirs[i] + '/X_mm.txt', delimiter=',')
            T = np.loadtxt(dirs[i] + '/T_stroboscopic.txt', delimiter=',')
            Z = np.loadtxt(dirs[i] + '/Z_mm_stroboscopic.txt', delimiter=',')
        ...
        #ax[i].set_xticks([-20, -15, -10, -5, 0, 5, 10, 15, 20])
    ...
    #plt.show()
    plt.savefig('ventanas_fernando.png', dpi=300)
    plt.close()