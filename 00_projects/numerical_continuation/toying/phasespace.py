...
if __name__ == '__main__':
    ...
    def F(phi1, phi2, gamma, nu, mu, k):
        ...
        NL01 = P ** 2 * (np.cos(phi1) * np.cos(phi2) ** 3 + np.sin(phi1) * np.sin(phi2) ** 3)
        NL02 = R ** 2 * (np.cos(phi1) * np.sin(phi1) ** 2 * np.cos(phi2) + np.cos(phi1) ** 2 * np.sin(phi1) * np.sin(phi2))
        return - k - nu * (P / R) * np.cos(phi1 - phi2) - (P / R) * (NL01 + NL02)


    def G(phi1, phi2, gamma, nu, mu, k):
        R = np.sqrt(((gamma + mu) / gamma) * (-nu + np.sqrt(gamma ** 2 - mu ** 2)))
        P = np.sqrt(R ** 2 * (gamma - mu) / (gamma + mu))
        NL01 = R ** 2 * (np.cos(phi2) * np.cos(phi1) ** 3 + np.sin(phi2) * np.sin(phi1) ** 3)
        NL02 = P ** 2 * (np.cos(phi2) * np.sin(phi2) ** 2 * np.cos(phi1) + np.cos(phi2) ** 2 * np.sin(phi2) * np.sin(phi1))
        return - k - nu * (R / P) * np.cos(phi1 - phi2) - (R / P) * (NL01 + NL02)
