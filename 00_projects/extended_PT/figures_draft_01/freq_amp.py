...
if __name__ == '__main__':
    ...
    #plt.scatter(dist[:-30], freq_dist[:-30])
    #plt.show()
    def F(x, A, c, b):
        #x = x.astype('complex128')
        return A * (((np.abs((x - c)) )** b))
    popt, pcov = curve_fit(F, dist[20:-30], freq_dist[20:-30], bounds=[(0, 26, 0), (5, 34, 1)])
    print(popt[2])
    plt.scatter(dist[20:-30], freq_dist[20:-30])
    plt.plot(dist[20:-28], F(dist[20:-28], popt[0], popt[1], popt[2]))
    plt.show()
    ...
    ax.set_aspect(0.45 * (xmax - xmin) / (ymax - ymin))
    ...
    plt.ylim([ymin, ymax])
    plt.xlim([xmin, xmax])
    plt.text(23.5, 0.25, "$\\textrm{Rabi Oscillations}$", fontsize=16)
    plt.text(32., 0.25, "$\\textrm{Non-interacting}$", fontsize=16)
    plt.grid(linestyle='--', alpha=0.5)
    plt.title("$\\times 10^{-2}$", loc='left', size="18")
    plt.tight_layout()
