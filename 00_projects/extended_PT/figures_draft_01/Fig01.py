...
if __name__ == "__main__":
    ...
    ax3.set_yticklabels([])
    ax3.set_xlabel("$x\ \\textrm{(mm)}$", fontsize=20)
    ...
    ax4.set_xlabel("$x\ \\textrm{(mm)}$", fontsize=20)
    ax4.set_ylabel("$t/T$", fontsize=20)
    ...
    ax4.text(xi_sim + 0.84 * (xf_sim - xi_sim), ti_sim + 0.87 * (tf_sim - ti_sim), "$\\textbf{(d)}$", fontsize=18, rotation=0, zorder=13, color="white")
