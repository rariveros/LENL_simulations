...
if __name__ == '__main__':
    ...
    for directory in directories:
        ...
        savedirectory = working_directory + "/" + directory + '/dist=20.000/'
        ...
        if m == 1:
            ...
            profiles_R.append(Z_modulo[I_inicial + IR, :] + m)
            profiles_L.append(Z_modulo[I_inicial + IL, :] + m)
        else:
            dist_ZR = []
            dist_ZL = []
            for j in range(len(IR_max)):
                dist_ZR.append(np.std(profiles_R[-1] - (Z_modulo[I_inicial + IR_max[j], :] + m)))
            for j in range(len(IL_max)):
                dist_ZL.append(np.std(profiles_L[-1] - (Z_modulo[I_inicial + IL_max[j], :] + m)))
            print(dist_ZR)
        ...
        ax2.plot(T, N_R, label="$N_{R}$", c="b", linewidth=2)
        ax2.plot(T, N_L, label="$N_{L}$", c="r", linewidth=2)
        ax2.set_xlabel("$t/T$", fontsize=25)
        ax2.set_xlim(0, t_max)
        ax2.legend(loc="upper right", fontsize=15)
        ax2.grid(alpha=0.3)
        plt.tight_layout()
        plt.savefig(savedirectory + '/time_series.png', dpi=300)
        plt.close()
        ...
        plt.savefig(savedirectory + '/module_spacetime.png', dpi=300)
        plt.close()

        pcm = plt.pcolormesh(X, T[IR:IR + I_period], Z_modulo[IR:IR + I_period, :], cmap=parula_map, shading='auto')
        cbar = plt.colorbar(pcm, shrink=1)
        cbar.set_label('$|A(x,t)|$', rotation=0, size=20, labelpad=-27, y=1.1)
        plt.xlim([X[0], X[-1]])
        plt.xlabel('$x$', size='20')
        ...
        plt.xlim(-50, 50)
        plt.grid(linestyle='--', alpha=0.5)
        plt.savefig(datafile + '/figures/gif/' + str(parameter) + '.png', dpi=300)
        plt.close()
    ...
    plt.tight_layout()
    plt.savefig(datafile + '/profiles_joydivision.png', dpi=300)
    plt.close()

    DATA = np.array(DATA)

    np.savetxt(datafile + '/data.txt', DATA, delimiter=',')