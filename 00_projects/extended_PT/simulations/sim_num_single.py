...
if __name__ == '__main__':
    ...
    project_name = '/PDNLS_bigaussian_num'
    ...
    phi = np.pi
    gamma_real = gamma_0 * (np.exp(- (x_grid - dist / 2) ** 2 / (2 * sigma ** 2)) + np.cos(phi) * np.exp(- (x_grid + dist / 2) ** 2 / (2 * sigma ** 2)))
    gamma_img = np.sin(phi) * np.exp(- (x_grid + dist / 2) ** 2 / (2 * sigma ** 2))
