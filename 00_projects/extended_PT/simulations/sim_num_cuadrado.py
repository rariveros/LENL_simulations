...
if __name__ == '__main__':
    ...
    project_name = '/PDNLS_bigaussian_num_cuadrado'
    ...
    dt = 0.01#0.0025
    T = 2500
    dx = 1#0.25 #en milimetros

    alpha = 6.524  #5.721
    beta = 1
    nu = 0.157#0.04812#0.0327449 #0.0052
    mu = 0.1
    gamma_0 = 0.258
    sigma = 15.8
    dist = 74.4
    ...
    for i in range(len(x_grid)):
        if np.absolute(x_grid[i]) < 20:
            gamma_r.append(1)
        else:
            gamma_r.append(0)
    gamma_real = gamma_0 * np.array(gamma_r)
    gamma_img = 0
