...
if __name__ == '__main__':
    ...
    project_name = '/parametric_snell/drift_tests'
    ...
    for gamma_0 in gammas:
        ...
        heviside = np.tanh(0.05 * x_grid)
        gamma_l = 1
        gamma_r = 1
        gamma_real = gamma_0 * (gamma_l * np.exp(- (x_grid - dist / 2) ** 2 / (2 * sigma ** 2)) + gamma_r * np.cos(phi) * np.exp(- (x_grid + dist / 2) ** 2 / (2 * sigma ** 2)))  + constant_gamma * heviside
        ...
        # Guardando gráficos
        plt.plot(x_grid, gamma_real, color="b")
        plt.plot(x_grid, gamma_img, color="r")
        plt.xlabel('$x\ \\textrm{(mm)}$', size='25')
        plt.xlim([x_grid[0], x_grid[-1]])
        plt.ylabel('$\gamma(x)$', size='25')
        plt.ylim([np.amin(gamma_real) * 1.1, np.amax(gamma_real) * 1.1])
