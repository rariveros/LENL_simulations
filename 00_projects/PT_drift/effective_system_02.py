...
if __name__ == '__main__':
    ...
    params_01 = np.loadtxt(directory + '/parameters.txt', delimiter=',')

    Z_r_01 = Z_r[-1, :]
    Z_i_01 = Z_i[-1, :]
    J_middle = np.argmin(np.abs(x_grid))
    PHI_L = Z_r_01[:J_middle] + 1j * Z_i_01[:J_middle]
    PHI_R = Z_r_01[J_middle:] + 1j * Z_i_01[J_middle:]

    PHI_R = np.append(np.zeros(J_middle), PHI_R)
    PHI_L = np.append(PHI_L, np.zeros(J_middle))
    phi_01 = PHI_L + 1j * PHI_R
    phi_02 = PHI_L - 1j * PHI_R
    #plt.plot(x_grid, np.real(phi_01), color="b", label="$phi_{+}^{R}$")
    #plt.plot(x_grid, np.imag(phi_01), color="r", label="$phi_{+}^{I}$")
    #plt.plot(x_grid, np.real(phi_02), color="b", linestyle="--", label="$phi_{-}^{R}$")
    #plt.plot(x_grid, np.imag(phi_02), color="r", linestyle="--", label="$phi_{-}^{I}$")
    plt.plot(x_grid, np.real(PHI_R), color="b")
    plt.plot(x_grid, np.imag(PHI_R), color="r")
    plt.plot(x_grid, np.real(PHI_L), color="b", linestyle="--")
    plt.plot(x_grid, np.imag(PHI_L), color="r", linestyle="--")
    plt.show()
    plt.close()
    for d in distances:
        X_L = - d / 2
        ...
        PHI_L = np.append(Z_r_00[Delta_J_L:], np.zeros(Delta_J_L)) + 1j * np.append(Z_i_00[Delta_J_L:],
                                                                                    np.zeros(Delta_J_L))
        ...
        cof = [Gamma_1, Gamma_2, Pi_11, Pi_12, Pi_21, Pi_22, Sigma_11, Sigma_12, Sigma_21, Sigma_22]
