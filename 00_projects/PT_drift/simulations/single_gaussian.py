...
if __name__ == '__main__':
    ...
    project_name = '/PT_drift/responses'
    ...
    for i in ies:
        for j in jes:
            alpha = 6.524  #5.721
            beta = 1
            nu = 0.02 #0.014 #0.04812#0.0327449 #0.0052
            mu = 0.1
            gamma_0 = i
            sigma = 10
            ...
            Nt = t_grid.shape[0]
            Nx = x_grid.shape[0]
            U_1_init = 0.01 * np.random.rand(Nx)#U1_light[-1, :]
            U_2_init = 0.01 * np.random.rand(Nx) #U2_light[-1, :]