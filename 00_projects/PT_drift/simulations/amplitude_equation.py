...
if __name__ == '__main__':
    ...
    project_name = '/PT_drift/amplitude_eq'
    ...
    for i in ies:
        ...
        psi_A = U1_light * np.exp(1j * Kc * x_grid) + np.conjugate(U1_light) * np.exp(-1j * Kc * x_grid)
        psi_B = U2_light * np.exp(1j * Kc * x_grid) + np.conjugate(U2_light) * np.exp(-1j * Kc * x_grid)
        psi = psi_A + 1j * psi_B
