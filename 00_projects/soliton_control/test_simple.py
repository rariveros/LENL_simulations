...
if __name__ == '__main__':
    ...
    nus = np.arange(-0.2, 0.0, 0.001)
    G = []
    for i in range(len(nus)):
        nu = nus[i]
        delta = - nu + np.sqrt(gamma ** 2 - mu ** 2)
        k = np.sqrt(2 / np.pi)
        Delta = np.sqrt(alpha / delta)#np.arccosh(2)/(np.sqrt(2 * np.log(2)))#(np.sqrt(np.pi / 2)) #1.68 #(np.sqrt(np.pi / 2))
        ...
        #print(sig_eff)
        #print(Delta)
        f = x_grid * np.exp(- x_grid ** 2 / (2 * sigma ** 2)) * (1 / np.cosh(x_grid / Delta))
        I = integrate.simpson(f, x_grid)
        #lambd = np.sqrt(2 / np.pi) * sigma ** 2 * k / (sigma ** 2 + sig_eff ** 2)#np.sqrt(2 * delta) * sigma ** 2 / (sigma ** 2 + np.pi * alpha / (2 * delta))
        cos2 = mu / gamma_eff
        G_i = (mu * I / (Delta * np.pi)) * np.ones(len(x_grid))#* x_grid * np.exp(- x_grid ** 2 /  (2 *(sigma ** 2 + sig_eff ** 2)))
        print(I)
        print(G_i[0])
        G.append(G_i)
    G = np.array(G)
    ...
    plt.savefig('naive_bifurcation.png', dpi=200)
    plt.close()