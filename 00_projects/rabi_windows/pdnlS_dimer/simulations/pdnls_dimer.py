...
if __name__ == '__main__':
    ...
    for nu in nus:
        for k in K:
            ...
            fields_init = [U_init, Uconj_init, V_init, Vconj_init]
            ...
            Uconj = np.array(fields_history)[:, 1]
            V = np.array(fields_history)[:, 2]
            Vconj = np.array(fields_history)[:, 3]
            ...
            np.savetxt(file + subfile + '/Uconj.txt', Uconj, delimiter=',')
            np.savetxt(file + subfile + '/V.txt', V, delimiter=',')
            np.savetxt(file + subfile + '/Vconj.txt', Vconj, delimiter=',')
            ...
            Uconj_light = Uconj[0::lightness]
            V_light = V[0::lightness]
            Vconj_light = Vconj[0::lightness]
            ...
            ax1.plot(np.real(U_light), np.imag(U_light), c="green", label="$z_2$")
