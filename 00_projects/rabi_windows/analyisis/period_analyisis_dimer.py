...
if __name__ == '__main__':
    ...
    for i in range(len(distances)):
        ...
        if len(tau_R) == 1 and np.amax(UR_Rs[i]) > 0.05:
            freq = np.infty
            freq_std = 0
        elif len(tau_R) == 1 and np.amax(UR_Rs[i]) < 0.05:
            freq = 0
            freq_std = 0
        else:
            freq = np.mean(np.diff(tau_R))
            freq_std = np.std(np.diff(tau_R))
        frequencies.append([freq, freq_std])
    frequencies = np.array(frequencies)
    np.savetxt(save_directory + '/periods.txt', frequencies, delimiter=',')