...
if __name__ == '__main__':
    ...
    for directory_01 in directories:
        ...
        modules = np.loadtxt(dir_01 + '/powers.txt', delimiter=',')
        ...
        #ax.plot(Ks, nus, 1000 * freqs[:, 0], c="k", zorder=1)
        ax.scatter(Ks, nus, modules[:, 0], c=colors, alpha=1.0, edgecolors="k", lw=1.0, s=20, zorder=5)
        ax.plot(Ks, nus, modules[:, 0], c="k", zorder=1)
    # --- tu vista intacta ---
    ax.view_init(22, -163, 0)
    ...
    ax.set_xlabel(r'$d$', fontsize=20)   # EJE K = kappa
    ...
    ax.tick_params(axis='both', which='major', direction="in", labelsize=13, length=6, width=1.6, pad=2)
    ax.tick_params(axis='z',    which='major', direction="in", labelsize=13, length=6, width=1.6, pad=2)
    # ========================================================
