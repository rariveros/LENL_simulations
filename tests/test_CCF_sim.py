...
if __name__ == '__main__':
    ...
    def CCF_wavelength_cos(x_grid, phi, k_min, k_max, dk, plot):
        ...
        for i in range(len(k_test)):
            phi_test_i = np.cos((2 * np.pi / k_test[i]) * x_grid)
    ...
    k_test_01, CCF_max_k_01, ind_max_01 = CCF_wavelength_cos(X[x_min:x_max], modulo[-1, x_min:x_center], 10, 50, 0.01, plot="NO")
    k_test_02, CCF_max_k_02, ind_max_02 = CCF_wavelength_cos(X[x_min:x_max], modulo[-1, x_center:x_max], 10, 50, 0.01, plot="NO")
    ...
    plt.plot(k_test_01, CCF_max_k_01, color="k")
    plt.plot(k_test_02, CCF_max_k_02, color="k")
    plt.scatter(k_test_01[ind_max_01], CCF_max_k_01[ind_max_01], c="r", zorder=10)
    plt.scatter(k_test_02[ind_max_02], CCF_max_k_02[ind_max_02], c="r", zorder=10)

    plt.xlim(10, 80)
    plt.grid(alpha=0.5)
    plt.tight_layout()
    plt.savefig(directory + "/CCF_wavelength.png", dpi=300)
