...
if __name__ == '__main__':
    ...
    d_nu = 0.05
    d_gammas = 0.05
    nus = [0.056]#np.around(np.arange(0.1, 0.6, d_nu), 3)
    gammas = [0.166]#np.around(np.arange(0.1, 0.6, d_gammas), 3)
    for nu_i in nus:
        for gamma_i in gammas:
            ...
            [tmin, tmax, dt] = [0, 500, 0.001]
            [xmin, xmax, dx] = [-200, 200, 0.2]
            ...
            # Initial Conditions Soliton
            #x_0 = 1
            #delta = np.sqrt(- nu + np.sqrt(gamma_0 ** 2 - mu ** 2))
            #print(np.sqrt(gamma_0 ** 2 - mu ** 2))
            #print(delta)
            #U_1_init = (np.sqrt(2) * delta) / np.cosh(delta * (x_grid - x_0)) ** 2
